...
public class TermVector extends Feature {
    ...
    private static final String MIN_FREQUENCY = "minFrequency"; //todo: min tf-idf
    ...
    public void buildTermDictionary(List<Document> documents) {
        if (!lexiconFilePath.isEmpty() || termDictionary != null) {
            return;
        }
    ...
    public TermDictionary buildNewTermDictionary(List<Document> documents) {
        ...
        if (minFrequency > 1) {
            td.eliminateByFrequency(minFrequency);
        }
    ...
    private void parseAndSetFrequencyTreshold() {
        String value = parameters.get(MIN_FREQUENCY);
        if (value != null)
            minFrequency = Integer.parseInt(value);
    }
    ...
    public String getDeclarationForArff() {
        ...
        for (String nGramName : termDictionary.getTerms()) {
            sb.append("@attribute ").
                    append(nGramName.replace(' ', '_').replace('\'', '^')).append("_").append(field).append(" ").
